/*
   ...
   public:
	   ...
	   */
	   void DrawCircle(int pFromX,int pFromY,int pRadius,uint8_t pRed,uint8_t pGreen,uint8_t pBlue,bool pFilled = false);
   ...
   {
   public:
	   // Caller has to delete the returned pointer. It's theirs now.
	   static const Font* AllocateSmallFont();
	   static const Font* AllocateBigFont();
   ...
   private:
	   // Object does NOT take ownership of pFontData as it may be a static. 
	   Font(const uint8_t* pFontData,int pCharWidth,int pCharHeight);
	   
	   const int mWidth,mHeight,mPitch,mCharSize;
	   const uint8_t* mData;
